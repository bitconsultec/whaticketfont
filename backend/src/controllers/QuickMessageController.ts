...
import FindNotCategoryService from '../services/QuickMessageService/FindNotCategoryService ';
...
export const findListNotCategory = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const params = req.query as FindParams;
  const records: QuickMessage[] = await FindNotCategoryService(params);
...
export const categoriesMessages = async (req: Request, res: Response): Promise<Response> => {
  const {companyId} = req.user
  //const params = req.query as FindParams;
  const records: QuickMessage[] = await FindCategoriesWithMessages(companyId);
  return res.status(200).json(records);
};
