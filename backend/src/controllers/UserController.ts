...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (!companyUser) {
    ...
    if(
      ...
      const companyExist = Company.findOne({where:{
        name: companyName
      }})

      if(companyExist){
        throw new AppError("ERR_COMAPANY_REGISTRED");
      }
    }
    ...
    try {
      ...
      if (
        ...
        function removeNineAfterDDD(phoneNumber: string) {
        ...
        const updatedNumber = removeNineAfterDDD(phone);

        await wbot.sendMessage(`${updatedNumber}@s.whatsapp.net`, {
          text: body
        });
      }
    } catch (error) {
...
export const remove = async (
  ...
  if (user.super) {
    return res
      .status(401)
      .json({ error: "Você não possui permissão para excluir o usuario super!" });
  }
