...
export const indexPlan = async (
  ...
  // console.log(`Pasta raiz da aplicação: ${rootFolder}`);

  // const folderPath = path.join(rootFolder, `/public/company${id}`); // Substitua 'sua-pasta' pelo nome da pasta
  // const info = await getFolderInfo(folderPath);

  // if (info) {
  //   console.log(`Tamanho total: ${info.totalSize} bytes`);
  //   console.log(`Total de arquivos: ${info.totalFiles}`);
  //   console.log(`Última atualização: ${info.lastUpdated}`);
  // }
  if (requestUser.super === true) {
