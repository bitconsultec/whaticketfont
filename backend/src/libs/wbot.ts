...
const waVersion = [2, 3000, 1019520164];
...
const getProjectWAVersion = async () => {
  try {
    ...
    // Filtra versões estáveis (beta: false) e ordena pela data de expiração
    const stableVersions = data.versions.filter(
      (version) => version.beta === false,
    );

    // Remove a versão mais recente (a atual)
    const sortedVersions = stableVersions.sort(
      (a, b) => new Date(b.expire).getTime() - new Date(a.expire).getTime(),
    );
    sortedVersions.shift(); // Remove a versão mais recente

    // Pega a versão anterior
    const previousVersion = sortedVersions[0];
    const versionArray = previousVersion.version
      .split(".")
      .map((v) => parseInt(v));

    return versionArray;
  } catch (error) {
    logger.warn("Failed to get current WA Version from remote URL");
  }
  return waVersion;
};
  ...
  return new Promise(async (resolve, reject) => {
    try {
      (async () => {
        ...
        const { id, name, allowGroup, companyId, provider } = whatsappUpdate;
        ...
        logger.info(`using WA v${version.join(".")}, isLatest: true`);
        ...
        let wsocket: Session = makeWASocket({
          ...
          auth: {
            creds: state.creds,
            keys: makeCacheableSignalKeyStore(state.keys, logger), // ✅ correto
          },
          generateHighQualityLinkPreview: true,
          linkPreviewImageThumbnailWidth: 192,
          shouldIgnoreJid: (jid) =>
            isJidBroadcast(jid) || (!allowGroup && isJidGroup(jid)),
          browser: [appName, "Desktop", ""],
  ...
  `,
        ...
        wsocket.ev.on(
          ...
          async ({ id: remoteJid, presences }) => {
            try {
              const lastPresence = presences[remoteJid]?.lastKnownPresence;
              const selfJid = wsocket?.user?.id;
              if (remoteJid === selfJid) return;
              if (!lastPresence) return;

              const contatoNumber = remoteJid.replace(/\D/g, "");
              const contact = await Contact.findOne({
                where: { number: contatoNumber, companyId: whatsapp.companyId },
              });
              if (!contact) return;
              ...
              // EMITE O EVENTO NORMAL
              const nsp = getIO().of(`/${whatsapp.companyId}`);
              nsp
              ...
              // **LOG SIMPLIFICADO**
              console.log(
                `[PRESENCE] contato=${contatoNumber} ticketId=${ticket.id} status=${lastPresence}`,
              );
            } catch (error) {
              logger.error(
                { remoteJid, presences },
              ...
              console.log("[BACKEND][ERRO no presence.update]", error);
            }
          },
        );
