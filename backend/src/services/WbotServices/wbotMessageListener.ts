...
const verifyQueue = async (
  ...
  if (queues.length === 1 && !chatbot) {
    ...
    try {
      const userQueue = await ListUserQueueServices(queues[0].id);

      if (userQueue.userId > -1) {
        randomUserQueueId = userQueue.userId;
      }
    ...
    if (queues[0].closeTicket) {
      ...
      //const IsbotActived =  queues[0]?.integrationId  && !ticket?.userId
      if (settings?.userRandom === "enabled" && randomUserQueueId) {
        await UpdateTicketService({
          ticketData: { queueId: queues[0].id, status: ticket.status === "lgpd" ? "pending" : ticket.status, userId: randomUserQueueId },
          ticketId: ticket.id,
          companyId
        });
      } else {
  ...
  const botText = async () => {
    ...
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      if (!choosenQueue.chatbots.length && choosenQueue.greetingMessage.length !== 0) {
        ...
        if (settings?.userRandom === "enabled" && randomUserId) {
          await UpdateTicketService({
...
export const handleRating = async (
  ...
  if (!isNil(complationMessage) && complationMessage !== "" && !ticket.isGroup) {
    ...
    await SendMessageService({ ticket, ticketTracking: ticketTraking, message: body });
  } else {
...
const withMedia = (mensagem: proto.IWebMessageInfo): boolean => {
  const hasMedia =
    mensagem.message?.imageMessage ||
    mensagem.message?.audioMessage ||
    mensagem.message?.videoMessage ||
    mensagem.message?.stickerMessage ||
    mensagem.message?.documentMessage ||
    mensagem.message?.documentWithCaptionMessage?.message?.documentMessage ||
    // mensagem.message?.extendedTextMessage?.contextInfo?.quotedMessage?.imageMessage ||
    // mensagem.message?.extendedTextMessage?.contextInfo?.quotedMessage?.videoMessage ||
    // mensagem.message?.extendedTextMessage?.contextInfo?.quotedMessage?.audioMessage ||
    mensagem.message?.ephemeralMessage?.message?.audioMessage ||
    mensagem.message?.ephemeralMessage?.message?.documentMessage ||
    mensagem.message?.ephemeralMessage?.message?.videoMessage ||
    mensagem.message?.ephemeralMessage?.message?.stickerMessage ||
    mensagem.message?.ephemeralMessage?.message?.imageMessage ||
    mensagem.message?.viewOnceMessage?.message?.imageMessage ||
    mensagem.message?.viewOnceMessage?.message?.videoMessage ||
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message?.imageMessage ||
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message?.videoMessage ||
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message?.audioMessage ||
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message?.documentMessage ||
    mensagem.message?.documentWithCaptionMessage?.message?.documentMessage ||
    mensagem.message?.templateMessage?.hydratedTemplate?.imageMessage ||
    mensagem.message?.templateMessage?.hydratedTemplate?.documentMessage ||
    mensagem.message?.templateMessage?.hydratedTemplate?.videoMessage ||
    mensagem.message?.templateMessage?.hydratedFourRowTemplate?.imageMessage ||
    mensagem.message?.templateMessage?.hydratedFourRowTemplate?.documentMessage ||
    mensagem.message?.templateMessage?.hydratedFourRowTemplate?.videoMessage ||
    mensagem.message?.templateMessage?.fourRowTemplate?.imageMessage ||
    mensagem.message?.templateMessage?.fourRowTemplate?.documentMessage ||
    mensagem.message?.templateMessage?.fourRowTemplate?.videoMessage ||
    mensagem.message?.interactiveMessage?.header?.imageMessage ||
    mensagem.message?.interactiveMessage?.header?.documentMessage ||
    mensagem.message?.interactiveMessage?.header?.videoMessage ||
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate?.documentMessage ||
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate?.videoMessage ||
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate?.imageMessage ||
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate?.locationMessage

  return !!hasMedia
};


const isMsgForwarded = (mensagem: proto.IWebMessageInfo): boolean => {
  const forwarded = mensagem.message?.extendedTextMessage?.contextInfo?.isForwarded ||
    mensagem.message?.imageMessage?.contextInfo?.isForwarded ||
    mensagem.message?.audioMessage?.contextInfo?.isForwarded ||
    mensagem.message?.videoMessage?.contextInfo?.isForwarded ||
    mensagem.message?.documentMessage?.contextInfo?.isForwarded
...
const handleMessage = async (
  ...
  try {
      ...
      if (!msg.key.fromMe) {
        ...
        if (enableLGPD && ticket.status === "lgpd" && !isImported) {
          ...
          if ((contact.lgpdAcceptedAt === null || settings?.lgpdConsent === "enabled") &&
            ...
            if (hasMedia) {
              mediaSent = await verifyMediaMessage(msg, ticket, contact, ticketTraking, false, false, wbot);
            } else {
    ...
    if (hasMedia) {
      mediaSent = await verifyMediaMessage(msg, ticket, contact, ticketTraking, isMsgForwarded(msg), false, wbot);
    } else {
      await verifyMessage(msg, ticket, contact, ticketTraking, false, isMsgForwarded(msg));
    }
    ...
    if (ticket.queue && ticket.queueId && !msg.key.fromMe && (ticket.status !== 'nps')) {
