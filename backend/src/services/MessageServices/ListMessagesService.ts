...
const ListMessagesService = async ({
  ...
  const { count, rows: messages } = await Message.findAndCountAll({
    where: { ticketId: tickets, companyId, mediaType:  { [Op.ne]: "reactionMessage"  }},
    ...
    include: [
      ...
      },
      {
        ...
        where:{
          mediaType:  { [Op.ne]: "reactionMessage" }
        },
        as: "quotedMsg",
        include: [
          {
      ...
      },
      {
        model: Message,
        as: "reactionsMsg",
        attributes: ["id", "fromMe", "body", "contactId", "createdAt"], // Escolha os atributos necessários
        where: {
          mediaType: "reactionMessage", // Filtro para incluir apenas mensagens do tipo "reaction"
        },
        required: false, // Permite mensagens sem reações
      },
      {
        model: Ticket,
        required: true,
